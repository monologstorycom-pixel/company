<x-admin-layout>
    <x-slot name="title">Edit Chatbot Rule</x-slot>

    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Edit Chatbot Rule</h1>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 max-w-3xl">
        <form action="{{ route('admin.chatbot.update', $chatbotRule) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" id="keyword" name="keyword" value="{{ old('keyword', $chatbotRule->keyword) }}"
                ...
                <div>
                    ...
                    <textarea id="response" name="response" rows="5"
                              ...
                              required>{{ old('response', $chatbotRule->response) }}</textarea>
                    @error('response')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select id="type" name="type"
                                ...
                                required>
                            <option value="text" {{ old('type', $chatbotRule->type) == 'text' ? 'selected' : '' }}>Text</option>
                            <option value="link" {{ old('type', $chatbotRule->type) == 'link' ? 'selected' : '' }}>Link</option>
                            <option value="product" {{ old('type', $chatbotRule->type) == 'product' ? 'selected' : '' }}>Product Reference</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" id="priority" name="priority" value="{{ old('priority', $chatbotRule->priority) }}" min="1" max="10"
                ...
                <div>
                    ...
                    <select id="status" name="status"
                            ...
                            required>
                        <option value="active" {{ old('status', $chatbotRule->status) == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $chatbotRule->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
